...
const NuclearnuclearsUnderConstruction = () => {
  ...
  const chartData = {
    labels: nuclearData?.data?.reactors_under_construction[0]?.data.map(
      (country) => country.name
    ),
    datasets: [
      {
        label: "Capacity (MWe)",
        data: nuclearData?.data?.reactors_under_construction[0]?.data.map(
          (country) => country.y
        ),
  ...
  return (
    <div className="py-16 px-6 md:px-20 md:pb-32 md:pt-10  min-h-screen">
...
export default NuclearnuclearsUnderConstruction;
